...
class OrderDetailTester extends StatefulWidget {
  ...
  static const String orderDetailTesterPageRouteName = 'order detail tester page route name';
...
class _OrderDetailTesterState extends State<OrderDetailTester> {
          ...
          listener: (ctx, state) async {
            if (state is LoadedSingleOrder) {
              ...
              addNotification(
                ...
                courierContent: 'You have confirmed arrival to ${state.order.tester_name} from ${state.order.sender_name}.',
                senderContent: 'Your specimen has arrived to ${state.order.tester_name}.',
                testerContent: 'Courier ${state.order.courier_name} has just arrived from ${state.order.sender_name}.',
            ...
            if (state is ErrorState) {
              ...
              addNotification(
                ...
                courierContent: 'Tester center ${state.order.tester_name} accepted the order from ${state.order.sender_name}',
                senderContent: 'Speciment sent to ${state.order.tester_name} via courier ${state.order.courier_name} has been deliverd successfully!',
                testerContent: 'You have accepted order sent from ${state.order.sender_name} transported by ${state.order.courier_name}.',
                courierAction: NotificationAction.NavigateToOrderDetalCourier,
                testerAction: NotificationAction.NavigateToOrderDetalTester,
                senderAction: NotificationAction.NavigateToOrderDetalSender,
                payload: {'orderId': widget.orderId},
                    ...
                    ? Align(
                        ...
                        child: Container(
                          ...
                          child: Stack(
                            children: [
                              Padding(
                                padding: const EdgeInsets.only(bottom: 100, left: 10, top: 10, right: 10),
                                              ...
                                              ? ListView.builder(
                                                  ...
                                                  itemBuilder: (ctx, index) {
                                                    return GestureDetector(
                                                      onTap: () {
                                                        Navigator.pushNamed(context, EditPatientInfoPage.editPatientInfoRouteName, arguments: {
                                                          'patient': state.order.patients![index],
                                                          'orderId': widget.orderId,
                                                          'index': index,
                                                          'canEdit': false,
                                                          'canAddResult': true,
                                              ...
                                              onTap: () async {
                                                showDialog(
                                                    ...
                                                    builder: (ctx) {
                                                      return AlertDialog(
                                                        title: Text('Confirm'),
                                                        content: Text('Are you sure you want to confirm courier arrival?'),
                                                        actions: [
                                                          TextButton(
                                                              onPressed: () {
                                                                ordersBloc.add(
            ...
            ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: order.patients![index].specimens![i].assessed ? Colors.green.withOpacity(0.2) : Colors.yellow.withOpacity(0.1),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: ListTile(
                    title: Text(
                    ...
                    subtitle: Text('ID : ${order.patients![index].specimens![i].id ?? ''}'),
                    trailing: !order.patients![index].specimens![i].assessed
                        ? TextButton(
                            onPressed: () async {
                              bool create = await showModalBottomSheet(
                                  ...
                                  context: context,
                                  builder: (ctx) {
                                    return AssessSpecimen(ctx, order.patients![index].specimens![i]);
                                  });
                              ...
                              if (create == true && order.patients![index].specimens![i].type == 'Sputum') {
                                ...
                                if (success) {
                                  addNotification(
                                    ...
                                    testerContent: 'You Accepted Sputum specimen for ${order.patients![index].name} from ${order.sender_name}',
                                    senderContent: '${order.patients![index].name}\'s Sputum Specimen have accepted by ${order.tester_name}.',
                                ...
                                if ('Mucoid Purulent' != sputumCondition) {
                                  addNotification(
                                    orderId: order.orderId!,
                                    testerContent: 'You Rejected Sputum specimen for ${order.patients![index].name} from ${order.sender_name}',
                                    senderContent: '${order.patients![index].name}\'s Sputum Specimen have been rejected by ${order.tester_name}.',
                                ...
                                if (success) {
                                  addNotification(
                                    orderId: order.orderId!,
                                    testerContent: 'You Accepted Stool specimen for ${order.patients![index].name} from ${order.sender_name}',
                                    senderContent: '${order.patients![index].name}\'s Stool Specimen is accepted by ${order.tester_name}.',
                                ...
                                if ('Formed' != stoolCondition) {
                                  addNotification(
                                    orderId: order.orderId!,
                                    testerContent: 'You Rejected Stool specimen for ${order.patients![index].name} from ${order.sender_name}',
                                    senderContent: '${order.patients![index].name}\'s Stool Specimen have been rejected by ${order.tester_name}.',
                                    content: 'One specimen got rejected by tester!',
                                    courier: false,
                                    testerAction: NotificationAction.NavigateToOrderDetalTester,
                                    senderAction: NotificationAction.NavigateToOrderDetalSender,
                                ...
                                if (success) {
                                  addNotification(
                                    orderId: order.orderId!,
                                    testerContent: 'You Accepted Urine specimen for ${order.patients![index].name} from ${order.sender_name}',
                                    senderContent: '${order.patients![index].name}\'s Urine Specimen is accepted by ${order.tester_name}.',
                                    content: 'One specimen got accepted by courier!',
                                    courier: false,
                                    testerAction: NotificationAction.NavigateToOrderDetalTester,
                                    senderAction: NotificationAction.NavigateToOrderDetalSender,
                        ...
                        : Icon(
                            order.patients![index].specimens![i].rejected ? Icons.close : Icons.check,
                          ),
                  ),
                );
              },
          ...
          Container(
            ...
            child: StatefulBuilder(builder: (context, ss) {
              return DropdownButtonHideUnderline(
                ...
                items: <String>['Yes, end to end', 'Yes, partly', 'No, not at all'].map((String value) {
              ...
              ? Container(
                  ...
                  child: StatefulBuilder(builder: (context, ss) {
                    return DropdownButtonHideUnderline(
                      ...
                      items: <String>['Mucoid Purulent', 'Bloodstreak', 'Saliva'].map((String value) {
                        return DropdownMenuItem<String>(
                          value: value,
                          child: Text(value),
                        );
                  ...
                  ? Container(
                      ...
                      child: StatefulBuilder(builder: (context, ss) {
                        return DropdownButtonHideUnderline(
                          ...
                          items: <String>['Formed', 'Unformed', 'Liquid'].map((String value) {
                            return DropdownMenuItem<String>(
                              value: value,
                              child: Text(value),
                            );
  ...
  Container buildPatients(BuildContext context, Patient patient, String orderId, int index, [bool isFromCourier = false]) {
    String message = '';
    int counter = 0;
